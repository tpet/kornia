...
def get_sift_pooling_kernel(ksize = 25):
    ...
    xc2 = ks_2 - (torch.arange(ksize).float() + 0.5 - ks_2).abs()  # type: ignore # noqa
    kernel = torch.ger(xc2, xc2) / (ks_2**2)
    return kernel


def get_sift_bin_ksize_stride_pad(patch_size,
                                  num_spatial_bins):
    """
...
class SIFTDescriptor(nn.Module):
    ...
    Args:
        ...
        rootsift: (bool): if True, RootSIFT (Arandjelovic et. al, 2012)
        is computed
    ...
    def forward(self, input):
        ...
        ang_bins = []
        for i in xrange(0, self.num_ang_bins):
